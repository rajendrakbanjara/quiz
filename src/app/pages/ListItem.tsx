...
interface ChildProps {
  ...
  const ListItem: React.FC<ChildProps> = ({lable, value, isSelected, changeOption, index}) => {
    return(
        <>
           <li onClick={()=>changeOption(index)} className="flex items-center font-bold text-slate-600 dark:text-white bg-slate-50 dark:bg-slate-600 p-1.5 w-[36rem] rounded-2xl text-2xl hover:border-purple-600 hover:border-4 border-4 border-transparent hover:transition border-opacity-0 hover:border-opacity-100 hover:duration-700">
