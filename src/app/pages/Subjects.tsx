          ...
          <div>
            <div>
              <ul className="space-y-4">
                <li onClick={()=> chooseSubject(1)}className="flex items-center font-bold text-slate-600 dark:text-white bg-slate-100 dark:bg-slate-600 p-2 w-[36rem] rounded-2xl text-2xl">
                  <div className="bg-orange-100 p-4 ml-2 my-2 rounded-md  mr-7 text-2xl">
                    <FaCode color="orange" size="25px" />
                  </div>
                   HTML
                </li>
                <li onClick={()=>chooseSubject(2) }className="flex items-center font-bold text-slate-600 dark:text-white bg-slate-100 dark:bg-slate-600 p-2 w-[36rem] rounded-2xl text-2xl">
                  <div className="bg-green-100 p-4 ml-2 my-2 rounded-md  mr-7 text-2xl">
                    <LuPaintbrush2 color="Green" size="25px" />
                  </div>{" "}
                  CSS
                </li>
                <li onClick={()=> chooseSubject(3)} className="flex items-center font-bold text-slate-600 dark:text-white bg-slate-100 dark:bg-slate-600 p-2 w-[36rem] rounded-2xl text-2xl">
                  <div className="bg-blue-100 p-4 ml-2 my-2 rounded-md  mr-7 text-2xl">
                    <RiJavascriptLine color="Blue" size="25px" />
                  </div>
                  JavaScritp
                </li>
                <li onClick={()=> chooseSubject(4)} className="flex items-center font-bold text-slate-600 dark:text-white bg-slate-100 dark:bg-slate-600 p-2 w-[36rem] rounded-2xl text-2xl">
